...
class RobloxManager(ctk.CTk):
	...
	def create_instance(self,instance_num,from_image):
		def task():
			...
			cmd=f"docker run -d --name {container_name} --privileged --cgroupns=host -v /sys/fs/cgroup:/sys/fs/cgroup:rw -v ./sober-logs-{instance_num}:/root/.var/app/org.vinegarhq.Sober/data/sober/sober_logs -p {port}:6080 --device /dev/dri --device /dev/snd --cpus=\"1.0\" --memory=\"256m\" --memory-swap=\"3g\" --shm-size=\"256m\" -e LIBGL_ALWAYS_SOFTWARE=1 -e GALLIUM_DRIVER=llvmpipe {image_name}"
